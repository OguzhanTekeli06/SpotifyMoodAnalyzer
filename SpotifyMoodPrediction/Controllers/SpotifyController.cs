    ...
    public async Task<ActionResult> GetRecentlyPlayed()
{
    ...
    {
        ViewBag.ErrorMessage = "Spotify API'sinden veriler alınırken bir hata oluştu.";
        return View("Error");
    }
    ...
    try
    {
        ...
        foreach (var item in jsonDocument.RootElement.GetProperty("items").EnumerateArray())
        {
            ...
            // Sadece şarkı adı ve sanatçı adı ekleniyor
            songs.Add(new
            {
                Name = name,
                Artist = artist
    ...
    catch (Exception ex)
    {
        ...
        ViewBag.ErrorMessage = "Spotify'dan alınan veriler işlenirken bir hata oluştu.";
        return View("Error");
    }
